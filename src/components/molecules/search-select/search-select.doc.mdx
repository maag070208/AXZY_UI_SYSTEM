...
| `renderOption` | `(option, state: { isSelected, searchTerm }) => ReactNode` | — | Custom template for each dropdown option. |
| `clearable` | `boolean` | `true` | Shows a clear (X) button inside the input. |
...
- **Custom options**: `renderOption` replaces the default option rendering and receives `{ isSelected, searchTerm }`.
- **Clear button**: an (X) button appears inside the input when there is a search term or a selected value; `clearable={false}` disables it. Clearing resets the search and selection, and calls `onSearch("")` / `onClear()`.
